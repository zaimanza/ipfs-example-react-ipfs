...
function App() {
  ...
  const [nodeIsOnline, setNodeIsOnline] = useState(null)
  ...
  useEffect(() => {
    ...
    const getNodeIsOnline = async () => {
      const nodeId = await ipfs.isOnline();
      console.log("HELLO_IS_ONLINE")
      console.log(nodeId)
      setNodeIsOnline(nodeId);
    }
        ...
        {(id || version) &&
          <section className='bg-snow mw7 center mt5'>
            ...
            <div className='pa4'>
              ...
              {nodeIsOnline && <div className='mb4' >
                <Title>{"isOnline"}</Title>
                <div className='bg-white pa2 br2 truncate monospace' >{nodeIsOnline ? 'Online' : 'Offline'}</div>
              </div>}
            </div>
          </section>
        }
